...
function ModifEntree() {
    ...
    useEffect(() => {
        axios.get(`${API_URL}/entry/` + id).then((res) => {
            setCategoriesForEntree(res.data.categories);
                    ...
                    return (
                        <div key={index} class="small-box-content flex-down">
                                <div class="flex-spaced">
                                    <input type="text" placeholder="Titre" value={donnee.title} onChange={(e) => handleMajDonneeTitle(e, index)} />
                                    <button class="float-right" onClick={() => handleSupprDonnee(index)}>x</button>
                                </div>
                                <textarea rows="10" value={donnee.contenu} onChange={(e) => handleMajDonneeContent(e, index)} ></textarea>
                        </div>
                    );
                })}
